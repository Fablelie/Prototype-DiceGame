...
//[RequireComponent(typeof(MeshFilter))]
    public class CurveMesh : MonoBehaviour {
        ...
        public Mesh GenerateMesh(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, float resolution = 0.1f) {
            var mesh = new Mesh();//GetMesh ();
            var shape = GetExtrudeShape ();
            var path = GetPath (p1, p2, p3, p4, resolution);
 
            return Extrude(mesh, shape, path);
        }
        ...
        private OrientedPoint[] GetPath(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, float resolution = 0.1f) {
        ...
        private Mesh Extrude(Mesh mesh, ExtrudeShape shape, OrientedPoint[] path) {
