...
public class Node : MonoBehaviour {
	...
	void Start() {
		...
		foreach(Node node in nodes){
			...
			string objectLineName = GetObjectLineName(this.nid, node.nid);
			...
			if (!objectLines.ContainsKey(objectLineName)) {
				...
				line.transform.rotation = Quaternion.LookRotation(node.transform.position - transform.position);//Quaternion.LookRotation(node.transform.position, Vector3.up);//(,//node.transform.position.x, transform.position.y, node.transform.position.z);
				line.transform.localScale = new Vector3(0.025f, 1f, (Vector3.Distance(transform.position, node.transform.position)-1.5f)/10);
	...
	void OnDrawGizmosSelected() {
		foreach(Node node in nodes){
			if (node != null) {
				...
				Gizmos.DrawLine(transform.position+new Vector3(0.01f,0,0.01f), node.transform.position+new Vector3(0.01f,0,0.01f));
				Gizmos.DrawLine(transform.position+new Vector3(-0.01f,0,-0.01f), node.transform.position+new Vector3(-0.01f,0,-0.01f));
	...
	void OnDrawGizmos() {
		foreach(Node node in nodes){
			if (node != null) {
				Debug.DrawLine(transform.position, node.transform.position, new Color(255, 255, 255, .25f));
			}
		}
		...
		Handles.Label(transform.position + Vector3.up * 2, "NID " + nid);
	}
