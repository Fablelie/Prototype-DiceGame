...
public class DokaponGameMode : GameMode {
	...
	void RouteToNode(Node target, Node node=null, Node pNode=null, int step=0, List<Node> result=null) {
		...
		foreach (Node n in node.nodes) {
			...
			RouteToNode(target, n, node, step, result2);
		}
	}
	...
	List<Node> PathToNode(Node target, Node node=null, Node pNode=null, int step=0, List<Node> result=null) {
		...
		foreach (Node n in node.nodes) {
			...
			Debug.LogError(String.Format(GetIndent(step)+"[END {0}] {1}", n.nid, GetNodeString(subResult)));
			...
			noEntry = false;
			Debug.LogWarning(String.Format(GetIndent(step)+"[ForwardCollect] {0} <- {1} == {2}", n.nid, node.nid, GetNodeString(subResult)));
